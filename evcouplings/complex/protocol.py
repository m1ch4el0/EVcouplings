...
def describe_concatenation(
    ...
    # as a pd.DataFrame
    annotations_1 = read_species_annotation_table(annotation_file_1)
    species_1 = annotations_1.species.values

    annotations_2 = read_species_annotation_table(annotation_file_2)
    ...
    species_overlap = list(set(species_1).intersection(set(species_2)))
    ...
    n_paralogs_2 = float(np.median(list(Counter(species_2).values())))
    ...
    if (
        genome_location_filename_1 is not None
        and genome_location_filename_2 is not None
    ):
...
def inter_species(**kwargs):
    ...
    Concatenates sequences by defined species-species pairs
    based on the best hit to the focus sequence
    in each species
    ...
    InvalidParameterError:
        Defined parameter do not match mandatory parameters
